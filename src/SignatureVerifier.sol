...
contract SignatureVerifier {
    ...
    //////////////////////////////////////////////////////////////*/
    /* 
     * People liked signatures, and wanted to use them to send transactions by signatues. Standard ETH transactions have the following components:
     * 
     * RLP<nonce, gasPrice, startGas, to, value, data>
     * 
     * So, if someone signed a transaction with these values, they could hypothetically send it to the network. We want to allow users to sign transactions to get this data, so others can send transactions for them. 
     * however, there is an issue. 
     * In our example above, we used this:
     * 
     * ecrecover(_hashedMessage, _v, _r, _s);
     * 
     * We assumed that the order was <hash, v, r, s>, but that was just because that's what this contract decided. Any contract could hypothetically put these in any order, and this would make it very difficult for wallets to display to the user what was going on!
     * 
     * So, we as a web3 community decided on a standard for encoding & decoding signatures. The format we chose looks like this:
     * 
     * 0x19 <1 byte version> <version specific data> <data to sign>.
     * 
     * 0x19 is a prefix saying "hey! I'm a signature!". 0x19 was chosen because it's a weird number not used in any other context. It's decimal value is 25, we don't really use 25 for anything. 
     * 
     * Additionally, this ensures that the data associated with a signed message cannot be a valid ETH transaction itself, because of how ETH transactions are encoded. There are some other reasons for this number as well. 
     * 
    ...
    /* 
     * EIP-191 was cool, but not enough detail. The "4-6 : Application specific data" of EIP-191 wasn't specific enough. On user wallets, when prompted to sign some structured data, they would just be shown some horrible bytestring/hex. So the community came together to structure that application specific data so users could know more clearly what they were signing. 
     * 
     * You'll notice, we still follow EIP-191, but now we have a way to format the data inside EIP-191.
     * 
     * In this EIP, we add a lot of structures to make verification of the data extensible and strict. For EIP-191, version 0 looked like this:
     * 
     ...
     * 
     * A domain separator is the hash of a struct which defines the domain of the message being signed. It contains one or all of the following:
     * 
     * string name
     * string version 
     * uint256 chainId
     ...
     * 
     * This way, contracts can know that a signature was created specificly for this contract or not. We wouldn't want a signature for a different contract to work on your contract! Knowing this, we can rewrite our EIP-712 data to be:
     * 
     ...
     * 
     * A hashStruct is just a hash of a struct, that includes a hash of what the struct looks like. The hash of the type of the struct is known as the typehash. 
     * 
     * 0x19 0x01 <hashStruct(eip712Domain)> <hashStruct(message)>
     * 0x19 0x01 <keccak256(keccak256(encodeType(typeOf(eip712Domain))) || eip712Domain)> <keccak256(keccak256(encodeType(typeOf(message))) ‖ message)>
     * 
     * well that's horrible to read, more simply, we can say:
     * 
     * 0x19 0x01 <hash of who verifies this signature, and what the verifier looks like> <hash of signed structured message, and what the sig looks like>
     * 
    ...
    */
    ...
    constructor() {
        ...
        eip_712_domain_separator_struct = EIP712Domain({
            ...
            verifyingContract: address(this) // ideally, you set this as "this", but you could make it whatever contract you want to use to verify signatures
        });
    ...
    /*//////////////////////////////////////////////////////////////
                  REPLAY RESISTANT EIP-712 SIGNATURES
    //////////////////////////////////////////////////////////////*/
}
