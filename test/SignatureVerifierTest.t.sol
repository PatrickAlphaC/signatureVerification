...
contract SignatureVerifierTest is Test {
    ...
    function testSignaturesCanBeReplayed() public {
        ...
        // Verify the message
        vm.prank(address(1));
        bool verifiedOnce = signatureVerifier.verifySigner712(message, v, r, s, user.addr);
        vm.prank(address(2));
        bool verifiedTwice = signatureVerifier.verifySigner712(message, v, r, s, user.addr);
    ...
    function testVerifySignatureReplayResistant() public {
        ...
        SignatureVerifier.ReplayResistantMessage memory messageStruct = getReplayResistantMessageStruct(message);

        // Verify the message
        vm.prank(address(1));
        bool verifiedOnce = signatureVerifier.verifySignerReplayResistant(messageStruct, v, r, s, user.addr);
        assertEq(verifiedOnce, true);

        vm.prank(address(2));
        vm.expectRevert();
        signatureVerifier.verifySignerReplayResistant(messageStruct, v, r, s, user.addr);
    }
    ...
    function getReplayResistantMessageStruct(uint256 message)
        public
        view
        returns (SignatureVerifier.ReplayResistantMessage memory)
    {
        // Find an unused nonce
        uint256 nonce = signatureVerifier.latestNonce(user.addr) + 1;

        return SignatureVerifier.ReplayResistantMessage({
            number: message,
    ...
    function _signMessageReplayResistant(uint256 message) internal view returns (uint8, bytes32, bytes32) {
        bytes1 prefix = bytes1(0x19);
        bytes1 eip191Version = bytes1(0x01); // EIP-712 is version 1 of EIP-191

        SignatureVerifier.ReplayResistantMessage memory messageStruct = getReplayResistantMessageStruct(message);
        bytes32 hashedMessageStruct =
            keccak256(abi.encode(signatureVerifier.REPLAY_RESISTANT_MESSAGE_TYPEHASH(), messageStruct));
